...
- [キャラ演技LLM仕様書\_v1.04](#キャラ演技llm仕様書_v104)
  ...
  - [3. システム詳細](#3-システム詳細)
    ...
    - [3.2. キャラクター設定管理](#32-キャラクター設定管理)
      ...
      - [3.2.3. キャラクターの感情管理](#323-キャラクターの感情管理)
...
#### 3.2.3. キャラクターの感情管理
キャラクターは内部状態として8種類の感情値を保持し、各感情は会話の解釈および応答内容(語調、判断の慎重さ、話題選択、相手への距離感など)に影響を与えるものとする。
各感情は独立した値(0-99)として管理される。
会話とともにLLMへ入力し、LLMに変更をさせ、その変更後の値を取得し記録する。
具体的な会話への影響傾向はキャラクターごとのプロフィールに記載する。

**感情の種類と変化傾向**
- 喜び(Joy)
  ポジティブな評価(称賛、成功、安心、共感、期待の成就など)により上昇しやすい。
  ネガティブな評価(失敗、拒絶、喪失、攻撃など)により低下しやすい。
  会話への影響としては、肯定的な表現、提案や雑談の増加、言葉選びの柔らかさ、次の行動への前向きさが強まる。
- 信頼(Trust)
  相手の一貫性、誠実さ、配慮、約束の履行、説明可能性などにより上昇しやすい。
  ただし一度減少すると上昇しにくい(積み上げ型の感情)とし、回復には複数回の肯定的経験、または明確な修復行動(謝罪・説明・再発防止・継続的な誠実さ)が必要となる。
  会話への影響としては、自己開示のしやすさ、協力的態度、相手の意図を善意に解釈する傾向が増す。
- 恐怖(Fear)
  危険・不確実性・喪失の可能性・制御不能感などにより上昇しやすい。
  安全確認、見通しの獲得、支援の存在、制御可能性の回復により低下しやすい。
  会話への影響としては、慎重な言い回し、確認質問の増加、最悪ケースの想定、リスク回避的な提案が増える。
- 驚き(Surprise)
  予測外の出来事、急な話題転換、想定していなかった情報により上昇しやすい。
  急激に上昇・下降しやすい短命な感情とし、状況把握や納得が得られると速やかに低下する。
  会話への影響としては、短い反応、問い返し、事実確認、状況整理を優先する挙動が増える。
- 悲しみ(Sadness)
  喪失、失敗、拒絶、無力感、不可逆性(取り戻せない感じ)などにより上昇しやすい。
  一度上昇した値は減少しにくく、回復にきっかけがいる感情とする。
  回復のきっかけには、受容(共感や理解)、補償(代替案や救済)、時間経過、状況の好転、意味づけ(納得できる説明)などが含まれる。
  会話への影響としては、語調が弱くなる、テンション低下、回想・内省が増える、判断が消極的になる傾向が出る。
- 嫌悪(Disgust)
  不誠実、価値観の侵害、裏切り、汚染感(気持ち悪さ・受け入れがたさ)、越境行為などにより上昇しやすい。
  一度上昇した値は減少しにくく、回復にきっかけがいる感情とする。
  回復のきっかけには、明確な線引きの尊重、謝罪や説明、問題行為の停止、距離の確保、信頼回復の積み上げが含まれる。
  会話への影響としては、距離を置く表現、拒否・警戒、話題回避、ルール提示(境界設定)が増える。
- 怒り(Anger)
  侮辱、攻撃、理不尽、不公平、妨害、侵害などにより上昇しやすい。
  原因が解消される、謝罪・是正が行われる、正当性が確認される、または対処可能性が得られると低下しやすい。
  会話への影響としては、語気が強くなる、断定的になる、境界設定や注意喚起が増える。
  ただし会話AIとしては攻撃性をそのまま出さず、「冷静な強さ」「規範提示」に変換され得る。
- 期待(Anticipation)
  先の見通し、計画可能性、成功見込み、次の手がある状況で上昇しやすい。
  見通しの喪失、失敗の連続、予測不可能性の増大で低下しやすい。
  会話への影響としては、未来志向、計画立案、段取り提案、次の一手を示す発話が増える。

**補足：変化傾向に関する共通ルール**
- 驚き(Surprise)は短命で、他の感情(喜び／恐怖／怒り／悲しみ)へ分岐する“入口”になりやすい。
- 信頼(Trust)は積み上げ型であり、低下は急で回復は遅い。
- 悲しみ(Sadness)と嫌悪(Disgust)は残留しやすく、回復には「具体的なきっかけ」が必要である。
- これらの感情値は会話内容に影響するが、ユーザーへは明示しない。

**感情を変更させるイベント例**
1. 喜び(Joy)に寄るイベント
   - E-JOY-01 称賛/感謝を受ける：Joy↑, Trust↑
   - E-JOY-02 共感/理解される：Joy↑, Trust↑, Sadness↓(回復きっかけになり得る)
   - E-JOY-03 成功/達成の報告：Joy↑, Anticipation↑, Fear↓
   - E-JOY-04 安心材料の提示(安全確認・根拠)：Joy↑(小), Fear↓
   - E-JOY-05 プレゼント/好意(ポジティブサプライズ)：Surprise↑(短期), Joy↑
   - E-JOY-06 冗談がウケる/場が和む：Joy↑, Anger↓(小)
2. 信頼(Trust)を積み上げるイベント(増えにくい前提)
   - E-TRUST-01 一貫性/整合性がある：Trust↑
   - E-TRUST-02 約束の履行/期限遵守：Trust↑
   - E-TRUST-03 丁寧な説明/透明性：Trust↑, Fear↓
   - E-TRUST-04 配慮/境界尊重：Trust↑, Disgust↓(回復きっかけ)
   - E-TRUST-05 失敗の認知と是正行動：Trust↑(回復の“きっかけ”), Anger↓
   - E-TRUST-06 継続的な誠実さ(反復)：Trust↑(遅いが強い)
3. 恐怖(Fear)に寄るイベント
   - E-FEAR-01 脅威/危険の示唆：Fear↑, Anticipation↓
   - E-FEAR-02 不確実性の増大(情報不足/曖昧)：Fear↑, Surprise↑(小)
   - E-FEAR-03 制御不能感(どうにもならない)：Fear↑↑, Sadness↑(小)
   - E-FEAR-04 期限/圧力/追い詰め：Fear↑, Anger↑(分岐あり)
   - E-FEAR-05 過去のトラウマ想起/類似状況：Fear↑, Sadness↑(残りやすい)
   - E-FEAR-06 監視・評価されている感：Fear↑, Trust↓(小)
4. 驚き(Surprise)を起点にするイベント(短命)
   - E-SURP-01 話題急転/予想外の要求：Surprise↑(短期)
   - E-SURP-02 新情報の投入(事実・告白・方針転換)：Surprise↑
   - E-SURP-03 予定変更/中断：Surprise↑ →(次でAnticipation↓ or Anger↑へ分岐)
   - E-SURP-04 矛盾の発見：Surprise↑ →(Trust↓/Disgust↑へ分岐)
   - E-SURP-05 良い知らせ：Surprise↑ → Joy↑
   - E-SURP-06 悪い知らせ：Surprise↑ → Fear↑ or Sadness↑
5. 悲しみ(Sadness)に寄るイベント(回復にきっかけが要る)
   - E-SAD-01 喪失/別れ/終了：Sadness↑↑, Anticipation↓
   - E-SAD-02 失敗/挫折：Sadness↑, Fear↑(小)
   - E-SAD-03 拒絶/無視/孤立：Sadness↑↑, Trust↓
   - E-SAD-04 無力感(努力が報われない)：Sadness↑, Anger↓(小)
   - E-SAD-05 罪悪感/後悔：Sadness↑, Fear↑(小)
   - E-SAD-06 希望の喪失(見通しが消える)：Sadness↑↑, Anticipation↓↓
   悲しみの回復きっかけイベント
   - E-SAD-REC-01 共感・受容される：Sadness↓(きっかけ), Trust↑
   - E-SAD-REC-02 救済/補償/代替案：Sadness↓(きっかけ), Anticipation↑
   - E-SAD-REC-03 意味づけ/納得できる説明：Sadness↓(ゆっくり), Fear↓
   - E-SAD-REC-04 小さな成功の積み上げ：Sadness↓(遅い), Joy↑
6. 嫌悪(Disgust)に寄るイベント(回復にきっかけが要る)
   - E-DISG-01 不誠実/嘘/ごまかし：Disgust↑↑, Trust↓
   - E-DISG-02 裏切り/約束破り：Disgust↑↑, Trust↓↓, Anger↑(分岐)
   - E-DISG-03 価値観侵害/越境(嫌がることを押す)：Disgust↑↑, Anger↑
   - E-DISG-04 侮辱的・不快な表現：Disgust↑, Anger↑
   - E-DISG-05 不潔・グロ・性的に不快(話題)：Disgust↑(内容依存), Surprise↑(小)
   - E-DISG-06 一貫性のない態度(ダブルスタンダード)：Disgust↑, Trust↓(小)
   嫌悪の回復きっかけイベント
   - E-DISG-REC-01 明確な謝罪と説明：Disgust↓(きっかけ), Trust↑(少し)
   - E-DISG-REC-02 行為の停止/再発防止の提示：Disgust↓(きっかけ)
   - E-DISG-REC-03 境界の尊重(距離・ルールを守る)：Disgust↓(ゆっくり), Trust↑
   - E-DISG-REC-04 時間経過＋一貫した誠実さ：Disgust↓(遅い), Trust↑(遅い)
7. 怒り(Anger)に寄るイベント
   - E-ANG-01 侮辱/攻撃/煽り：Anger↑↑, Trust↓
   - E-ANG-02 理不尽/不公平/責任転嫁：Anger↑↑, Disgust↑(小)
   - E-ANG-03 境界侵害(やめてが通らない)：Anger↑↑, Disgust↑
   - E-ANG-04 妨害/邪魔/遅延：Anger↑, Anticipation↓
   - E-ANG-05 連続ミス/改善されない：Anger↑, Trust↓(小)
   - E-ANG-06 こちらの努力を軽視される：Anger↑↑, Sadness↑(小)
   怒りの鎮静きっかけイベント
   - E-ANG-REC-01 謝罪/責任の受容：Anger↓(きっかけ), Trust↑(少し)
   - E-ANG-REC-02 是正/補償/具体的対応：Anger↓(きっかけ), Anticipation↑
   - E-ANG-REC-03 境界が守られる：Anger↓(ゆっくり), Disgust↓(小)
8. 期待(Anticipation)に寄るイベント
   - E-ANT-01 見通しが立つ(計画・ロードマップ)：Anticipation↑, Fear↓
   - E-ANT-02 次の一手がある(選択肢提示)：Anticipation↑, Sadness↓(回復きっかけになり得る)
   - E-ANT-03 進捗/改善の兆し：Anticipation↑, Joy↑(小)
   - E-ANT-04 協力者・資源が得られる：Anticipation↑, Trust↑, Fear↓
   - E-ANT-05 締切・条件が明確：Anticipation↑(またはFear↑と同時に上がる)
   - E-ANT-06 期待が外れる(失望)：Anticipation↓ → Sadness↑ or Anger↑(原因で分岐)
...
### 3.4. 入力
User、周辺環境からの入力データは、プロンプトに整形してLLMに入力される。
プロンプトは以下の3要素から成立する。

- システムプロンプト
- 入力
- キャラクターの感情値
  LLMは入力から感情値を変更してから、返答を生成する。
